...
URL_LOGOUT=http://10.3.8.211/F.htm
URL_RELOGIN=http://10.3.8.211/a11.htm
...
login()
{
#DDDDD=username&upass=pass&savePWD=0&0MKKey=
#Cookie: myusername=xxxxxx; pwd=xxxxxx; username=xxxxxx; smartdot=000000

#	RESULT=`curl --silent -d "DDDDD=$USERNAME&upass=$PASSWD&AMKKey=" --data-urlencode "passplace=密码 Password" --cookie "myusername=$USERNAME; pwd=$PASSWD; username=$USERNAME; smartdot=000000" "$URL_RELOGIN"`

	RESULT=`curl --silent -d "DDDDD=$USERNAME&upass=$PASSWD&savePWD=0&0MKKey=" "$URL_LOGIN"`
        echo login OK!
}
